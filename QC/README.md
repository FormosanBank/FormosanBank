...
# Self-evaluate on the already-labeled XMLs (file-level, train=test apparent accuracy).
PYTHONPATH=. python -m QC.utilities.dialect_detector evaluate [--language ami]
```

The committed models live under `QC/utilities/dialect_models/` and are regenerated by `train`. The evaluation is deliberately train-on-test (overfitting accepted) because the tool is informational and retrains as the corpus grows; treat the numbers as *apparent* accuracy, not held-out. Note that orthographically near-identical dialects (e.g. the four "common-orthography" Amis dialects) are separated only by word/lexical evidence and may legitimately come back `unknown` on short or genre-neutral text.
