...
- **`original`**: the text as it appeared in the **actual original source** (the printed book, the web page, etc.) — *not* necessarily identical to a raw scrape/OCR pass. The goal is fidelity to the real source text, so OCR and scraping errors **should** be corrected (by hand-edit or script) to match what the source actually prints; punctuation may also be normalized. The one thing that must be preserved is the source's **spelling** (its orthographic letter choices). So: fixing `u`→`ʉ` because the OCR misread a barred vowel, or restoring a dropped final period, keeps the tier *more* faithful and is expected; silently changing spelling, or dropping a real letter/punctuation the source has, is the error to catch.
