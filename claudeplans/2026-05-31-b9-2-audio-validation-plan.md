...
- **W5** [PARTIAL] — `QC/validation/validate_audio_quality.py` scaffolded with mocked tests. Pipeline runs structurally (heavy deps installed 2026-06-01: `torch`/`torchaudio`/`allosaurus`/`Levenshtein`/`unidecode` via `requirements-audio-mt.txt`). Real-data probe surfaced a documented external blocker: the acoustic pass imports `utils_CTC` from a `data_quality_eval/` directory expected as a sibling of Jacob Ye's `Formosan-ILRDF_Dicts/data_validation/compute_metrics.py`. That directory does not exist in any branch of `Formosan-ILRDF_Dicts` locally or upstream — it's effectively an undistributed internal dependency. Probe runs (2026-06-01): `ePark` → "No entries found under Corpora/ePark/XML/ with audio" (audio not downloaded; XML is fine); `Paiwan_Stories` → same (XML has no per-S `<AUDIO>`); `TangRecordingsOfTaroko` → same (audio is at TEXT level, not S level); `ILRDF_Dicts` → found 3 entries, ran wav2vec2 imports successfully, then crashed at `ModuleNotFoundError: utils_CTC`. **Acceptance criterion "produces a scores CSV from `Corpora/ePark/`" NOT satisfied** — requires either (a) obtaining `utils_CTC` from Jacob Ye and downloading ePark audio, or (b) vendoring the small CTC helpers into FormosanBank.
