...
This audit reviewed every data-touching scraper/build transformation, mapped it to the current FormosanBank pipeline, rebuilt all output, ran the canonical validators, and compared final original tiers against a fresh source-derived build.
...
1. Parse the expanded ILRDF colloquial-video listing.
2. Scrape current page metadata, ISO code, raw ILRDF/ePark dialect payload, subtitles, translations, and start times.
...
- 468 current pages scraped; all carry ISO and resolved dialect metadata.
- 104,385 subtitle rows inspected: 104,049 retained; 1 sentence-initial `*`, 41 exact non-speech notes, and 294 Mandarin-only rows excluded.
...
| Check | Result |
|---|---|
| Unit tests in dev repo | 9 passed |
...
| Source-to-final reproducibility | 104,049/104,049 originals identical after canonical cleaning |

The audio validator was corrected upstream to calculate rate checks from each clip's `start/end` interval rather than the duration of the whole shared media file. Its regression suite passes.
