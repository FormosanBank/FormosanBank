...
**Status:** [DONE 2026-06-01] — W1–W11 all landed on `feature/claude-tooling-phase-0`. W1–W2 + W4–W9 + W11 bundled into user's `5355d81cc` commit; W3 brainstorm signed off 2026-06-01; W10 landed via merge of `worktree-agent-a06a0296f2869bc08` (13 per-rule commits V127–V139, +52 tests). Total V-codes used by B9.4: V110–V116 (W1/W2 ports), V120–V126 (W4–W9 user-locked TR1–TR7), V127–V139 (W10 brainstorm-derived TR8–TR21). V140 is taken by B9.2's `flag_audio_suspicious` (V120→V140 bump 2026-06-01).
...
- **W10** [DONE 2026-06-01] — 13 new rules V127–V139 landed via merge of `worktree-agent-a06a0296f2869bc08` (single merge commit on `feature/claude-tooling-phase-0`). Per-rule commits: V127 `7f5206434` (TR8 HARD smart quotes both tiers), V128 `0711a536b` (TR10 HARD control chars), V129 `cb4ae5611` (TR11 HARD asterisk in standard), V130 `f054ad184` (TR15 HARD whitespace), V131 `9400804b9` (TR16 HARD zero-width/BOM), V132 `f6022c78a` (TR9 SOFT HTML entities), V133 `d2fb79f2e` (TR12 SOFT dash in S-standard), V134 `e923421e2` (TR13 SOFT angle brackets), V135 `e9ac482ad` (TR14 SOFT trailing-punct mismatch), V136 `22c17e090` (TR18 SOFT mixed-script Latin/Cyrillic/Greek), V137 `5997a90cb` (TR19 SOFT trailing-decimal footnote), V138 `58e86502f` (TR20 SOFT superscript-digit), V139 `9c732c29d` (TR21 SOFT bracketed-digit). Test count: 80 pass on `tests/validators/test_validate_text.py` (was 28 before W10; +52 new). Subagent decisions worth noting: V128 control-char tests exercise the helper `_disallowed_control_chars` directly because lxml/libxml2 refuses to load most C0 controls (parse error before the rule ever sees them); V136 mixed-script scoped to FORM only and to the {Latin, Cyrillic, Greek} confusable set (CJK + other scripts NOT flagged); V137 uses `(?<!\d)\.\d+$` negative lookbehind so `3.14` is not flagged; V139 (bracketed-digit) deliberately does NOT require the "glue to non-digit" guard because standalone `[1]` is exactly the citation marker we want to catch.
