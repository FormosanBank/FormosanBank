...
def parse_morphemes(form: str, gloss: str) -> list[tuple[str, str]]:
    ...
    # Mark each infix extraction site with '-' in the base form so the root
    # morpheme records where the infix was removed: 'ʃ<uŋ>kaʃ' -> root 'ʃ-kaʃ'
    # (+ infix '-uŋ-'), matching the 'k-uda'/'-em-' convention in published
    # glossed corpora. A NUL sentinel marks the site through the '-'/'=' split
    # so it is not mistaken for a morpheme boundary, then becomes '-'. The gloss
    # side keeps deleting infix groups (the root gloss is not site-marked).
    form_values, form_boundaries = _split_base_morphemes(
        re.sub(r"<[^<>]+>", INFIX_SITE, form)
    )
    form_values = [value.replace(INFIX_SITE, "-") for value in form_values]
