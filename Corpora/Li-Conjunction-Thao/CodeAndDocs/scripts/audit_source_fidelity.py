...
from build_xml import correct_source_typos
...
FIXED_CHECKS = {
    ...
    "li2014_thao_S003": {
        ...
        "w_count": 6,
        "word": ("q<m>aʃiʃi", "sift<AF>", (("q-aʃiʃi", "sift"), ("-m-", "<AF>"))),
    },
    "li2014_thao_S005": {
...
def main() -> None:
    ...
    # Match the build: correct Blust-dictionary typos in the original tier before
    # comparing against the generated XML.
    for row in records.values():
        row["original"] = correct_source_typos(row["original"])
    ...
    for sentence_id, expected in FIXED_CHECKS.items():
        ...
        assert transl(word) == expected_gloss
        actual_morphemes = tuple(
            (forms(node)["original"], transl(node))
    ...
    for number, sentence_id in enumerate(records, start=1):
        ...
        if row["gloss"]:
            ...
            assert [transl(word) for word in words] == source_glosses
        else:
