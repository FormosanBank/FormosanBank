...
Run after clean_xml.py (so the generated splits are covered too) and before
standardize/add_phonology, so a morpheme this step removes never acquires a
standard FORM or a PHON tier. Idempotent: a second run reports 0/0.

Note the interaction with C012: ``standardize.py`` applies its hyphen step
only to sentences that still have an ``M`` tier, so a sentence stripped here
keeps any segmentation hyphen in its standard FORM. That is the documented
C012 M-guard gap, not a Yedda-specific choice.
