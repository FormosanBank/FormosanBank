...
- **14 `V121` findings remain HARD.** They are the article's own optional-material parentheses inside word and morpheme FORMs, across seven words: `(n)aku-yakuyab-yab`, `puken-(en)`, `manuyung-(e)`, `(n)u-kipung`, `(u)m-lavi`, `chi-ka-(y)bubu`, and `puken-ku(a)` — plus `sira(unctn)`, where the parenthesis is the article's uncertainty marker rather than optional material. POL-026 would turn each into two sentences; whether to do that here is a separate question from the slash ruling and has not been decided, so the source notation stands. (`V121` was 28 before this rebuild; `Kangkang/S18`'s `kan(u)` is the one case resolved rather than left as notation.)
