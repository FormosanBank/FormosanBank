...
- **`CodeAndDocs/`**
  ...
  - `resolve_standard_parentheses.py` — pipeline step 5; resolves the article's `( )` notation in the standard tier.
...
5. **Resolve the article's parentheses in the standard tier** — `CodeAndDocs/resolve_standard_parentheses.py`. The article's key gives `( )` two jobs and they need opposite treatment, and neither can survive into a standard tier, because no orthography has a parenthesis.

   **Optional material** (`puken-(en)`, `(u)m-lavi`, `ku(a)`) is a real alternation: the word reads with the bracketed letters or without. Choosing either silently asserts what the article declines to say, so **the word gets both**, through the same mechanism as the source's slash alternations — `FORM[@kindOf="standard"]` takes the fuller reading, closest to the printed letter sequence, and `FORM[@kindOf="alternate"]` takes the reading without. **7 words.** The alternate sits on the word and only there: not on the morpheme, where a wholly-optional morpheme would have to alternate with nothing, and not on the sentence, for the same reason the slash alternates stop at the word. Morphemes simply lose the brackets and keep their letters (**6**).

   **The uncertainty marker `(unctn)`** is not text — it is the article's note that a reading is doubtful — so it is dropped from the standard tier rather than transliterated (**2**). Left in, the vowel rules rewrite it and `sira(unctn)` reaches the standard tier as `sira(onctn)`.

   The original tier keeps all of this notation untouched; it is the article's own and belongs in a faithful record of it.
