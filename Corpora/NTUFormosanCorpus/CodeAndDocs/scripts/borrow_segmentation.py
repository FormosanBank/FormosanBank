...
def convert(text, mp):
    # NOTE: models the PRE-2026-08 add_phonology output (no contextual
    # rules, punctuation kept). On XML whose PHON was generated by the
    # current pipeline, guard 5 (PHON reproducibility) fails for words
    # containing punctuation or null markers and those repairs are
    # skipped; review the "skip: PHON not reproducible" count after a
    # rerun. make.sh's final add_phonology pass canonicalizes PHON.
    mappings, cdict = mp
    return apply_phonology_mappings(text, mappings)
