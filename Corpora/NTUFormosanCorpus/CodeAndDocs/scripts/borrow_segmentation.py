...
def try_repair_w(w, language, index, mp, stats):
    ...
    for m, piece in zip(ms, pieces):
        # Original tier only: standard FORM is regenerated by standardize.py and
        # standard PHON by add_phonology.py. The two guards above deliberately
        # still REQUIRE both tiers -- they are safety checks, not writes.
