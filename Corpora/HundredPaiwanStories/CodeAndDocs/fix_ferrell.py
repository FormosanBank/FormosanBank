...
ORIGINAL tier's context. Every '?' or quote character in an original FORM is
classified:

  QUOTE    an apostrophe/single-quote character — always punctuation.
  QPUNCT   a '?' that is punctuation: it is immediately followed by a quote
           character (question inside quoted speech), or it is string-final
...
def classify(text: str, tier: str, *, is_last_w: bool = False,
             s_final_is_punct: bool = False) -> list[str]:
...
def process_file(path: Path, report: list, dry_run: bool) -> int:
    ...
    for s in root.iter("S"):
        ...
        s_punct = s_ends_in_question(s_form.text or "") if s_form is not None else False
        ws = s.findall("W")
        elements = [(s, "S", False)]
        for w in ws:
        ...
        for el, tier, is_last in elements:
            ...
            if tier == "W" and not is_last and orig.text.rstrip().endswith("?"):
