...
The previous output used the following proposed derivation. Its profile and
conversion table are still absent from current main; their presence in the
corpus PR does not establish a merged source-orthography decision. Current
conversion checking reports an unresolved `u` to `[o|u]` equivalence and `d`/`o`
coverage questions. The authorial article alone does not resolve the full table.
Keep the vowel distinction and resolve the shared inputs before final QC.
...
- standard FORM: `Orthographies/ConversionTables/Amis_Safolu_113.tsv`, Coastal
  column (the old `--single-pass` flag is no longer a current CLI option);
