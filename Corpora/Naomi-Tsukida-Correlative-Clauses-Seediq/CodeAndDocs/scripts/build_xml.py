...
def add_word_structure(
    ...
    for word_index, (word, gloss) in enumerate(aligned_words, start=1):
        ...
        source_gloss_attributes = {"xml:lang": "eng"}
        if canonical_gloss:
            source_gloss_attributes["kindOf"] = "original"
        ET.SubElement(
            word_element,
            "TRANSL",
            source_gloss_attributes,
        ).text = gloss
        if canonical_gloss:
            ET.SubElement(
                word_element,
...
def write_xml(tree: ET.ElementTree, path: Path = XML_PATH) -> None:
