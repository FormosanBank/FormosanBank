...
DIRECT_CHECKS = ROOT / "evidence" / "direct_source_checks.csv"
SUMMARY = ROOT / "evidence" / "source_alignment_summary.json"
...
def conversion_table() -> list[tuple[str, str]]:
    return [
        ...
        for row in build_xml.read_tsv(ROOT / "raw_data" / "source_to_ortho113.tsv")
    ]
...
def check_word_structure(sentence: ET.Element, record: dict[str, str]) -> None:
    ...
    for word_index, (word, (original, gloss)) in enumerate(
        ...
        require(
            [
                (translation.get("kindOf", ""), translation.text or "")
...
def main() -> None:
    ...
    require(len(root.findall(".//W")) == 201, "Expected 201 W elements")
    require(len(root.findall(".//M")) == 268, "Expected 268 M elements")
    require(
        all(word.findall("M") for word in root.findall(".//W")),
        "Every W must have at least one M in this segmented file",
    )
    ...
    require(
        {record["source_id"] for record in records}.issubset(
    ...
    for parent in root.findall(".//FORM/.."):
        ...
        for kind in ("original", "standard"):
            phonology = parent.findall(f"PHON[@kindOf='{kind}']")
            require(len(phonology) == 1, f"Expected one {kind} PHON at {parent.get('id')}")
            phon_text = phonology[0].text or ""
            require(phon_text.strip(), f"Empty {kind} PHON at {parent.get('id')}")
            require("*" not in phon_text, f"PHON placeholder at {parent.get('id')}")
    ...
    require(standard_form_count == 497, "Expected 497 FORM/PHON parents")
    require(len(root.findall(".//PHON")) == 994, "Expected 994 PHON tiers")

    final_files = sorted(
        path.relative_to(build_xml.CORPUS_ROOT).as_posix()
        for path in (build_xml.CORPUS_ROOT / "XML").rglob("*.xml")
    )
    require(
        final_files
    ...
    summary = {
        ...
        "created_sentence_variants": 4,
        "final_morphemes": 268,
        "final_phonology_tiers": 994,
        "final_sentences": 28,
        "final_words": 201,
    ...
    print("Source alignment passed: 39 units, 11 pages, 28 S, 201 W, 268 M, 994 PHON.")
