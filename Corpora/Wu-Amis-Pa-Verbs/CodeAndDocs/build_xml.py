#!/usr/bin/env python3
"""Build the public Wu (2006) Amis corpus from its reviewed source table."""
...
FINAL_XML = ROOT / "XML/Amis/pa-verbs.xml"
...
def write_tree(tree: ET.ElementTree) -> None:
    ...
    tree.write(
        FINAL_XML, encoding="utf-8", xml_declaration=True, short_empty_elements=False
    )
    data = FINAL_XML.read_bytes()
    if not data.endswith(b"\n"):
        data += b"\n"
    FINAL_XML.write_bytes(data)


def main() -> None:
    rows = read_examples()
    write_tree(build_tree(rows))
    print(f"Built {len(rows)} source-adjudicated sentences in {FINAL_XML}")
