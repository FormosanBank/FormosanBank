...
def test_refresh_keep_audio_leaves_audio_dir(tmp_path):
    corpus = tmp_path / "Corpora" / "Mini"
    xml_dir = corpus / "XML" / "Amis"; xml_dir.mkdir(parents=True)
    (xml_dir / "a.xml").write_text(
        '<?xml version="1.0" encoding="utf-8"?>'
    ...
    refresh.refresh_corpus(corpus, keep_audio=True, download=fake_download,
                           regen_stats=lambda d: None, computed_at="2026-06-12")
    assert (corpus / "Audio").exists()  # preserved (keep_audio=True)


def test_refresh_download_failure_propagates_and_keeps_partial(tmp_path):
    import pytest
    corpus = tmp_path / "Corpora" / "Mini"
    (corpus / "XML").mkdir(parents=True)
    # A pre-existing (partial) Audio/ dir that must NOT be deleted on download failure.
    (corpus / "Audio").mkdir()
    ...
    with pytest.raises(RuntimeError):
        refresh.refresh_corpus(corpus, keep_audio=False, download=boom,
                               regen_stats=lambda d: None)
    assert (corpus / "Audio").exists()  # download failed before try/finally -> not cleaned
