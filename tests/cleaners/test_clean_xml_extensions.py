...
def test_C012_canonical_null_morpheme_stripped_from_standard(
    tmp_path, fixtures_dir, copy_fixture
):
    """C012 strips canonical U+2205 from only the standard S surface."""
    work = copy_fixture(
        fixtures_dir / "c012_null_morpheme_in_standard_trv.xml", tmp_path
    )
    source = work.read_text(encoding="utf-8").replace("Ø", "∅")
    ...
    assert _form_texts_with_kindof(work, "S", "original") == [
        "∅-dhuq sapah ka tama da."
    ]
